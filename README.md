...
可視化は `run-loop.sh` のターミナル出力だけです。誰が（`ORCH` / `FIX` / `VERIFY`）何をしたかが
色分けで1行ずつ流れます。会場で読ませたいときは間を空けます：

```bash
STEP_DELAY=1 bash scripts/run-loop.sh
```

**agentモードでは claude の各ステップも細かく出ます。** `claude` を `--output-format stream-json --verbose`
で動かし、`run-loop.sh` 内のフォーマッタが Read / Edit / Bash / 結果 / VERDICT を `FIX` `VERIFY` 行として
逐次表示します（`💬` 発話 / `🔧` ツール / `↩` 結果 / `✓` 完了）。simモードでは決定論的な実況を出します。
